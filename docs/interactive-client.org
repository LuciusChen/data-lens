...
| =C-c C-c=     | Commit all staged changes (INSERT/UPDATE/DELETE) |
| =C-c C-k=     | Discard staged change at point                |
| =C-c C-p=      | Preview execution (staged batch if any; otherwise effective query) |
...
=E-<n>= for staged edits, =D-<n>= for staged deletions, and =I-<n>= for staged inserts.
...
connection header.
Tip: staged changes can be previewed with =C-c C-p= and copied/saved from the
...
staged inserts.  Header-based imports map by column name; otherwise clutch maps
...
12. C-c ' on a staged insert row → re-open that staged insert with its previous values
...
15. C-c C-k                   → discard staged change at point
16. g with staged changes     → prompts "Discard staged changes and re-run query?"
...
prepared/bound values under the hood; JDBC uses literal SQL until the sidecar
supports prepared execution.
...
In the result buffer, the *Edit* group provides the staged-commit workflow:
=i= stages an insert, =d= stages a deletion, =C-c C-k= discards the staged
change at point, and =C-c C-c= commits all staged changes.
