...
## JDBC Integration Rules

When debug mode is off:

- JDBC should still preserve enough structured failure data to surface a short,
  useful user error
- no heavy debug payload should be requested

When debug mode is on:

- JDBC requests may ask for backend debug payload
- JDBC agent stderr should be folded into the single debug buffer
- users should not need to inspect `*clutch-jdbc-agent-stderr*` directly

## Delete Targets

The following UI concepts should be removed rather than preserved behind
aliases:

- `clutch-show-error-details`
- `clutch-error-details-mode`
- `clutch-error-details-refresh`
- `clutch-error-details-copy-message`
- `clutch-error-details-copy-all`
- `clutch-error-details--fetcher`
- `clutch-error-details--details`
- user-facing wording that tells users to reopen/refresh a details buffer

The following may remain as internal plumbing only:

- `clutch-jdbc--agent-stderr-buffer`
- `clutch-jdbc--agent-stderr-tail`
- `*clutch-jdbc-agent-stderr*`
...
### Phase 1 — Freeze the UX

- add this design note
- mark `065` as superseded in spirit for the UI direction
- stop expanding `clutch-show-error-details`

### Phase 2 — Red tests

Write failing tests proving the old model is gone:

- no `clutch-show-error-details`
- no fallback to unrelated buffers
- debug information is only inspectable through the debug buffer command
- JDBC stderr is surfaced there, not as a separate user workflow

### Phase 3 — Implementation

- introduce the dedicated debug buffer/mode
- route all failure sources into shared problem/debug registries
- delete the old details command and UI path
...
- `README.org`
- `PRD.md`
- any troubleshooting references

Only document the single debug-buffer workflow.

## Why Deletion Matters Here

This is a workflow convergence change, not an additive feature.

Keeping the previous details UI around would preserve exactly the ambiguity that
this redesign is meant to remove.  The project guide is explicit about:

- converging UX
- deleting instead of deprecating
- avoiding hidden fallback behavior

So the correct implementation is replacement, not coexistence.
