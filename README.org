...
| =A=            | Aggregate numeric values (region/current cell; =C-u= refine rows/cols in region) |
| =]=            | Next column page                              |
...
Tip: use =A= after =C-x SPC= to aggregate one selected numeric column quickly.
...
9. W          Enter empty string → clears filter, restores original query
#+end_src
