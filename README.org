...
(add-to-list 'load-path "/path/to/mysql")
(add-to-list 'load-path "/path/to/pg-el")
(add-to-list 'load-path "/path/to/clutch")
(require 'clutch)
#+end_src

When installed from MELPA, =mysql= and =pg= are pulled in as package
...
mapping (mysql / pg / sqlite), and cross-backend value formatting and null
handling.
