...
verification defaults and CA trustfile configuration.  For MySQL, =:tls nil=
forces plaintext and suppresses the automatic MySQL 8 TLS retry path.
MySQL connections also accept =:ssl-mode disabled= as a compatibility spelling;
=off= remains accepted as an alias.
...
For MySQL, explicit =:tls nil= or =:ssl-mode disabled= is a strict opt-out from
TLS and auto-retry; use it only when the server actually permits non-TLS
authentication.  =off= remains accepted as a compatibility alias.
