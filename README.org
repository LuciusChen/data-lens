...
| =A=            | Aggregate numeric values (region/current cell; one summary over selected cells; =C-u= exclude rows/cols by index spec) |
| =]=            | Next column page                              |
...
| =c=            | Copy... (tsv/csv/insert; region = selected block, no region = current cell; =C-u= exclude rows/cols by index spec) |
...
Tip: with active region, =C-u c= / =C-u A= prompts:
=Exclude rows= then =Exclude columns=.
Index spec supports =6=, =6,8=, =10-12=, =1,3-5=; empty input means none.
