...
- SQL pagination (=LIMIT/OFFSET=) with fixed header and status bar
  - =tab-line=: row count, page, elapsed time, sort state, staged-change summary, aggregate status, and active filters (with =nerd-icons=, Unicode fallback)
...
- =clutch-query-timeout-seconds= controls database-side statement timeout for JDBC queries (default: 30s).
- =clutch-jdbc-rpc-timeout-seconds= controls Emacs <-> JDBC agent RPC timeout (default: 30s).
- MySQL/PostgreSQL connections can override via =:connect-timeout= and =:read-idle-timeout=.
