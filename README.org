...
| =C-c C-k=     | Discard pending deletion/insertion at point   |
...
of the result buffer is replaced with a colour-coded hint for the duration of
the session.
...
3. Fill values, C-c C-c to stage (C-c C-k to cancel)
                               staged rows appear as green ghost rows in result buffer
4. Use normal Emacs region selection to select rows
5. d                          → stage current row or selected region for deletion
                               staged rows are highlighted in red with strikethrough
6. C-c C-k                    → discard pending deletion or insertion at point
7. g with pending changes     → prompts "Discard pending changes and re-run query?"
8. C-c C-c in result buffer  → confirmation prompt showing all statements
...
(setq clutch-connection-alist
      '(;; Oracle (service name style)
        ...
        ;; Oracle (full JDBC URL override — SID style, or RAC)
        ("prod-oracle-sid" . (:backend oracle
                               :url "jdbc:oracle:thin:@db.corp.com:1521:ORCL"
                               :user "scott" :pass-entry "prod-oracle"))
        ...
        ;; Generic JDBC (custom URL)
        ("clickhouse" . (:backend oracle          ; any registered driver symbol
...
=ob-clutch.el= provides Org-Babel source block execution for MySQL, PostgreSQL,
and SQLite.  Results are returned as Org tables.

** Activation

#+begin_src emacs-lisp
;; Load the integration (adds handlers for mysql, postgresql, sqlite blocks)
(require 'ob-clutch)
#+end_src

Alternatively the per-language shims can be loaded individually:
=ob-clutch-mysql.el=, =ob-clutch-pg.el=, =ob-clutch-sqlite.el=.

** Header Arguments

| Argument       | Description                                      |
|----------------+--------------------------------------------------|
| =:backend=     | Database backend: =mysql=, =pg=, or =sqlite=     |
| =:host=        | Database host (default: =127.0.0.1=)             |
| =:port=        | Database port (default: backend default)         |
| =:user=        | Database user                                    |
...
#+begin_src org
,#+begin_src mysql :host 127.0.0.1 :user root :database mydb :pass-entry mydb-root
...
#+begin_src org
,#+begin_src postgresql :host 127.0.0.1 :user postgres :database mydb :pass-entry mydb-pg
...
#+begin_src org
,#+begin_src sqlite :database /path/to/mydb.sqlite
