...
#+begin_src
C-c C-t                     ;; List all tables
C-c C-j                     ;; Browse object (select object, insert SELECT into console)
TAB                          ;; Expand/collapse table columns (type, PK, FK)
E / C                        ;; Expand all / Collapse all
RET                          ;; Show DDL for table at point
v                            ;; Browse object at point (insert SELECT into console)
g                            ;; Refresh
#+end_src
...
Example: invoke =C-c C-j= (browse object), navigate to a candidate, then press
...
- Dictionary objects exposed only through =PUBLIC SYNONYM= (for example
  ...
  cache / schema browser snapshot.  That keeps Oracle system objects from
  flooding =C-c C-j= and the schema browser for ordinary query-console use.
...
- After connect (and after schema-affecting DDL), Oracle/JDBC also starts a
  ...
  using the prefix fast path, while =C-c C-j= / the schema browser / the
...
Press =C-c ?= to open the transient menu.  The *Schema* group contains =List tables=, =Describe table=, and =Browse object=.
