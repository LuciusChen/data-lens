...
- SQL pagination (=LIMIT/OFFSET=) with fixed header and status bar
  - =tab-line=: row count, schema state, page, elapsed time, sort state, staged-change summary, aggregate status, and active filters (with =nerd-icons=, Unicode fallback)
...
- MariaDB 10.11 has been live-validated through the native =mysql= backend;
  other MariaDB versions are expected to be broadly compatible but are not yet
  part of the regular test matrix
