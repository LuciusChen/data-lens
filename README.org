...
| =C-c C-j=   | Jump to object and run its default action |
| =C-c C-d=   | Describe object at point, or prompt |
| =C-c C-o=   | Show object actions at point, or prompt |
...
when possible; otherwise it opens a flat annotated object picker.  The picker
...
protocol implementation.  It delegates SQL execution and metadata queries to
=clutch-jdbc-agent=, a minimal JVM sidecar process that speaks a
one-JSON-object-per-line protocol on stdin/stdout.  The agent maintains
separate primary and metadata JDBC sessions for each logical clutch connection,
so background introspection does not contend with foreground SQL execution.
Java 17+ is required.  If the configured =java= is too old for the current
=clutch-jdbc-agent= jar, clutch reports a startup error instead of a generic
...
Press =C-c ?= to open the transient menu.  The *Objects* group contains =Jump object=, =Describe object=, =Object actions=, and =Refresh schema=.
