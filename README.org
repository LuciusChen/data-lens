...
| =C-c ;=     | Execute statement at point (=;=-only delimiter; blank lines preserved) |
...
On first =clutch-connect= to a JDBC backend, clutch will prompt to
...
- =clutch-jdbc-cancel-timeout-seconds= controls how long clutch waits for a JDBC
  =cancel= acknowledgement (default: 5s).  This is intentionally shorter than
  the normal JDBC RPC timeout; if cancel does not finish in time, clutch
  degrades to disconnecting the current connection instead of blocking longer or
  killing the agent.
- =clutch-jdbc-disconnect-timeout-seconds= controls how long clutch waits for a
  JDBC =disconnect= acknowledgement (default: 5s).  A stuck disconnect detaches
  the local connection state without killing the shared JDBC agent.
...
See =CLAUDE.md= §Pre-Commit Checklist for exact test commands.
...
For MySQL, =:ssl-mode disabled= is the explicit opt-out from TLS and auto-retry;
use it only when the server actually permits non-TLS authentication.  =off=
and =:tls nil= remain accepted as compatibility forms.
...
For PostgreSQL, =:sslmode prefer= is the closest match to libpq's default
negotiation style.  =:sslmode require= and =:sslmode verify-full= both force
TLS; the latter uses the current =pg-tls-verify-server= setting for hostname
and certificate verification.  =:tls t= and =:tls nil= remain available as
shorthand for =require= and =disable=.
