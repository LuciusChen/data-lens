...
;; Add the native backend packages you actually use:
;; (add-to-list 'load-path "/path/to/mysql")  ; for :backend mysql
;; (add-to-list 'load-path "/path/to/pg-el")  ; for :backend pg
(require 'clutch)
#+end_src

When installed from MELPA, install =clutch= plus the native backend packages
you actually use:
...
=:backend= and then asks for the matching connection fields.
