...
- Use =C-c C-a= to toggle between manual-commit and auto-commit at any time.
  When switching from manual→auto, the database commits any pending transaction
  per the JDBC specification; clutch prompts for confirmation when there are
  uncommitted changes.
- The console header-line shows =Tx: Manual= when in manual-commit mode.  Once
  uncommitted DML is present it changes to =Tx: Manual*= (highlighted) as a
  persistent reminder.
