...
business schemas such as =zj_test= and =cjh_test= within one connection, and
MySQL connections map schema switching to =USE database=.  ClickHouse is
handled separately: it has databases rather than runtime schemas, so =C-c C-l=
prompts with =SHOW DATABASES= and reconnects to the selected =:database=
instead of attempting session-level schema switching.
