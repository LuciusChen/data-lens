...
- External native protocol dependencies:
  - =mysql-wire= — pure Elisp MySQL wire protocol client
...
- SQL completion: table/column names scoped to the current statement, with native MySQL/PostgreSQL hot paths staying cache-first and queueing background column preheat on cache miss instead of blocking typing; JDBC backends keep their direct metadata path; completion case can be customized via =clutch-sql-completion-case-style=
- Eldoc: three-tier lookup — (1) schema-aware: table name shows database, column count, and table comment; column name shows =table.column=, type, nullability, PK/FK references, and comment; schema metadata is cache-first and quietly preheated in the background on cache miss, so point motion does not block; (2) live MySQL =HELP= lookup (MySQL only, result cached per session); (3) static reference covering ~150 SQL keywords and functions with call signatures and descriptions (works without a connection; MySQL/PG-specific entries annotated)
...
- Schema refresh state is explicit: MySQL, PostgreSQL, and JDBC start a background schema snapshot after connect/reconnect, while SQLite keeps its lightweight synchronous path; query console buffer names surface =schema...= / =schema~= / =schema!= / =schema 42t= style state markers; here =Nt= means "the current schema-cache snapshot contains N browseable table/object names". =schema 0t= is therefore a successful empty snapshot, not an error. Schema errors also appear in the console header-line (see below)
- Native MySQL/PostgreSQL also preheat passive metadata in the background: result buffers render immediately and then enrich cached column details, while object warmup keeps non-table categories off the first-open path. If Emacs thread primitives are unavailable, clutch falls back to the older synchronous native metadata path instead of disabling metadata features outright
...
- Per-table/per-column result displayers can override cell text via =clutch-register-column-displayer= while keeping the raw cell value, value viewer (=v=), and cell face logic unchanged
...
(add-to-list 'load-path "/path/to/mysql-wire")
(add-to-list 'load-path "/path/to/pg-el")
(add-to-list 'load-path "/path/to/clutch")
(require 'clutch)
#+end_src

When installed from MELPA, =mysql-wire= and =pg= are pulled in as package
dependencies.  For Org-Babel support, install the separate =ob-clutch=
package.
...
Execute SQL directly in org files via the separate =ob-clutch= package.  It
...
Compatibility entry files live in the =ob-clutch= package:
...
For =mysql-wire=, =pg=, and SQLite connection examples, TLS notes, timeout
...
=clutch-db-live-p= identity check, RPC timeout kill, native backend adapter
mapping (mysql-wire / pg / sqlite), and cross-backend value formatting and null
handling.

The pure protocol packages (=mysql-wire= and =pg=) and the Org-Babel bridge
