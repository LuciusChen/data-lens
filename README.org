...
- Eldoc: hover over a table name shows database, column count, and table comment; hover over a column name shows =table.column=, type, nullability, PK/FK references, and column comment; hover over a SQL function or keyword shows its call signature and description (works without a connection; ~110 functions covered, MySQL/PG-specific ones annotated)
...
=:password= is optional. When omitted (or nil), the password is looked up
via Emacs's built-in =auth-source= library, which supports:
...
#+begin_src emacs-lisp
;; Enable auth-source-pass (built into Emacs 26+)
(require 'auth-source-pass)
(auth-source-pass-enable)
#+end_src

Store the password with =pass=:

#+begin_src sh
pass insert mysql/127.0.0.1
#+end_src

=auth-source-pass= searches for entries matching the host and user.
See [[https://www.gnu.org/software/emacs/manual/html_node/auth/The-Unix-password-store.html][the auth-source manual]] for entry naming conventions.
...
Alternatively, open a plain SQL editing buffer with =M-x clutch-mode= and
connect manually with =C-c C-e=.
