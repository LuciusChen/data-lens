...
- Console header-line shows live connection context: database-type icon + backend name + connection key, plus =schema~= / =schema!= warnings when the schema cache needs attention, and =Tx: Manual= / =Tx: Manual*= when the connection is in manual-commit mode
