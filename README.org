...
- SQL pagination (=LIMIT/OFFSET=) with fixed header and status bar
  - =mode-line=: row count, page, elapsed time, sort state, staged-change summary, aggregate status, no-PK guidance, active filters, and cursor position (=R-N:column=) (with =nerd-icons=, Unicode fallback)
...
For JDBC failures, use =M-x clutch-show-error-details=.  The detailed
troubleshooting workflow is documented below in the interrupt/error handling
section.  Inside the details buffer, use =w= to copy the raw backend error and
=W= to copy the full rendered report.  When you need a deeper trace, enable
=M-x clutch-debug-mode=, reproduce the failure, then reopen the same details
buffer.
...
| =C-c ?=     | Transient menu             |
| =M-.=       | Jump to table alias definition in FROM/JOIN clause |
...
- After a JDBC failure, run =M-x clutch-show-error-details= from the relevant
  buffer.  This is the supported troubleshooting path for JDBC errors.  The
  details view is strict to the current buffer context: connected buffers show
  details for their current connection, while unconnected buffers show their
  own last structured JDBC failure.  Early startup validation errors such as a
  missing agent jar or Java executable still surface inline at the command
  boundary.  When the failing path involved
  hidden/internal SQL, the same details view also shows the generated SQL text
  that triggered the failure together with the related JDBC agent stderr tail.
  Use =w= inside that buffer to copy the raw backend message, or =W= to copy
  the full rendered diagnostics report for issue filing.
- =M-x clutch-debug-mode= is the opt-in deep-debug workflow and is off by
  ...
  connect/query/interrupt events for the current buffer context, and JDBC also
  requests an additional redacted backend debug payload (currently stack trace +
  request context).  The same =clutch-show-error-details= buffer shows that
  extra capture; no separate debug UI is required.
