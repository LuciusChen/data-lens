...
- Console header-line shows live connection context: database-type icon (with brand color) + backend name + connection key, plus =schema~= / =schema!= warnings when the schema cache needs attention, and =Tx: Auto= / =Tx: Manual= / =Tx: Manual*= for the current commit mode; header text is indented to align with the buffer text area (respects =display-line-numbers-mode=)
...
picker.  It opens the shared describe buffer for the target object.
...
- *=completing-read= minibuffer* — current candidate in =C-c C-d= / =C-c C-o=
