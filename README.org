...
| =Y=            | Copy cell value (prefix =C-u=: copy region cells as rectangular TSV block) |
| =C=            | Copy... (choose format: tsv / csv / insert; prefix =C-u= applies mode-specific behavior) |
| =v=            | View JSON cell (pretty-printed in new buffer) |
| =w=            | Copy row(s) as INSERT (supports region; prefix =C-u=: choose columns) |
| =y=            | Copy row(s) as CSV (supports region; prefix =C-u=: choose columns) |
| =e=            | Export (csv / csv-all / csv-file / csv-all-file) |
