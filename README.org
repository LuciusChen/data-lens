...
6. C-c .                      → fill current date/time field with "now"
...
9. C-c ' on a green ghost row → re-open that staged insert with its previous values
...
12. C-c C-k                   → discard pending deletion or insertion at point
