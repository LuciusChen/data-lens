...
sidecar (=clutch-jdbc-agent=, Java 21+).
...
The JDBC backend requires Java 21+ and =clutch-jdbc-agent.jar= (auto-downloaded on first use).
...
- Java 21+ for the JDBC backend (Oracle, SQL Server, DB2, Snowflake, Redshift)
...
one-JSON-object-per-line protocol on stdin/stdout.  Java 21+ is required.
...
(clutch-jdbc-install-driver 'oracle)      ; ojdbc11.jar + orai18n.jar from Maven Central
