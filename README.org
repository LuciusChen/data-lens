...
- Inline cell editing with staged UPDATE generation
- Staged INSERT (=i=), DELETE (=d=), and pending-batch preview/commit
...
- Per-table/per-column result displayers can override cell text via =clutch-register-column-displayer= or =clutch-column-displayers= while keeping the raw cell value, value viewer (=v=), and cell face logic unchanged
...
faces, and the value viewer (=v=) still use the normal clutch pipeline.
