...
- Schema refresh state is explicit: eager backends refresh on connect, lazy backends start stale, and query console buffer names surface =schema...= / =schema~= / =schema!= / =schema 42t= style state markers
...
| =C=            | Jump to column                                |
| =g=            | Re-execute query                              |
| =C-c '=       | Edit cell value / re-edit staged insert row   |
...
| =C-c C-p=      | Preview SQL (current effective query / pending update / selected delete) |
...
| =C-c .=              | Fill current date/time field with "now"         |
| =C-c C-c=            | Stage new row / update staged insert            |
| =C-c C-k=            | Cancel                                          |

Field labels are read-only and may show tags such as =generated=,
=default=..., =enum=, =bool=, =json=, and =required=.  =C-c C-c= validates
obvious enum / bool / json / numeric / date-time mistakes before staging.
...
3. TAB / S-TAB                → move between fields
                               field labels are read-only and can show tags like [enum], [json], [default=...]
4. M-TAB / C-M-i              → complete enum / bool-like fields (falls back to a chooser if needed)
5. C-c .                      → fill current date/time field with "now"
6. C-c C-c                    → validate locally, then stage (C-c C-k cancels)
                               staged rows appear as green ghost rows in result buffer
                               missing generated/defaulted columns render as <generated> / <default>
7. C-c ' on a green ghost row → re-open that staged insert with its previous values
8. Use normal Emacs region selection to select rows
9. d                          → stage current row or selected region for deletion
                               staged rows are highlighted in red with strikethrough
10. C-c C-k                   → discard pending deletion or insertion at point
11. g with pending changes    → prompts "Discard pending changes and re-run query?"
12. C-c C-c in result buffer → confirmation prompt showing all statements
